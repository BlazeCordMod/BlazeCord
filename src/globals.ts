...
export const blazecordGlobalExports = (window.__blazecord_exports = lazyValue(
    () => require("#globals#").default as Record<string, any>,
));

// This is cursed but hey, how else?
export function blazecordGlobalObject() {
    const availableModules = Object.keys(blazecordGlobalExports);
        ...
        return new Proxy(baseObject, {
            get(_, key: string) {
                ...
                if (matchingModulePaths.length === 1 && key === matchingModulePaths[0])
                    return blazecordGlobalExports[currentPath + key];
                ...
                for (const childPath of immediateChildren) {
                    ...
                    defineLazyProperty(childModules, childKey, () =>
                        createModuleProxy(blazecordGlobalExports[fullChildPath], `${fullChildPath}/`),
                    );
                }

                const currentModule = blazecordGlobalExports[currentPath + key] ?? {};
